...
def display_summary_metrics(summary_metrics):
    ...
    for idx, row in summary_metrics.iterrows():
        summary_listbox.insert(tk.END, f"{row['Metric']}: {row['Value']:.2f}")
...
def create_visualization_frame():
    ...
    fig.add_trace(go.Scatter(x=combined_trade_log['timestamp'], y=combined_trade_log['price'], mode='lines', name='Price'))
    fig.update_layout(title="Trade Log", xaxis_title="Date", yaxis_title="Price")

    trade_log_fig = go.FigureWidget(fig)
    trade_log_canvas = trade_log_fig.to_image(format="png")
    trade_log_img = ImageTk.PhotoImage(Image.open(BytesIO(trade_log_canvas)))
    trade_log_label = ttk.Label(viz_frame, image=trade_log_img)
    trade_log_label.image = trade_log_img
    trade_log_label.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))

    fig_summary = make_subplots(rows=1, cols=1)
    fig_summary.add_trace(go.Bar(x=summary_metrics['Metric'], y=summary_metrics['Value'], name='Summary Metrics'))
    fig_summary.update_layout(title="Summary Metrics", xaxis_title="Metric", yaxis_title="Value")

    summary_fig = go.FigureWidget(fig_summary)
    summary_canvas = summary_fig.to_image(format="png")
    summary_img = ImageTk.PhotoImage(Image.open(BytesIO(summary_canvas)))
    summary_label = ttk.Label(viz_frame, image=summary_img)
...
frame = ttk.Frame(backtest_frame, padding="10")

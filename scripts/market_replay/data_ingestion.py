...
def process_file(file_path, output_directory):
    ...
    try:
        ...
        print(f"Before conversion: df_l1['Price'] dtype = {df_l1['Price'].dtype}, df_l1['Volume'] dtype = {df_l1['Volume'].dtype}")
        print(f"Before conversion: df_l2['Price'] dtype = {df_l2['Price'].dtype}, df_l2['Volume'] dtype = {df_l2['Volume'].dtype}")

        # Ensure Price and Volume columns are strings before replacing commas
        print("Converting Price and Volume columns to string...")
        df_l1['Price'] = df_l1['Price'].apply(lambda x: str(x))
        df_l1['Volume'] = df_l1['Volume'].apply(lambda x: str(x))
        df_l2['Price'] = df_l2['Price'].apply(lambda x: str(x))
        df_l2['Volume'] = df_l2['Volume'].apply(lambda x: str(x))

        # Log data types after conversion to string
        print(f"After conversion to string: df_l1['Price'] dtype = {df_l1['Price'].dtype}, df_l1['Volume'] dtype = {df_l1['Volume'].dtype}")
        print(f"After conversion to string: df_l2['Price'] dtype = {df_l2['Price'].dtype}, df_l2['Volume'] dtype = {df_l2['Volume'].dtype}")

        # Replace commas with dots in Price and Volume columns
        print("Replacing commas with dots in Price and Volume columns...")
        ...
        print(f"After replacing commas: df_l1['Price'] dtype = {df_l1['Price'].dtype}, df_l1['Volume'] dtype = {df_l1['Volume'].dtype}")
        print(f"After replacing commas: df_l2['Price'] dtype = {df_l2['Price'].dtype}, df_l2['Volume'] dtype = {df_l2['Volume'].dtype}")
